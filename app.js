...
const con = mysql.createConnection({
    ...
    database: 'unichwmn_salesorderhelper'
});
...
app.get('/suggestions/item-master/:searchText', (req, res) => {
    ...
    for (var i = 0; i < keywords.length; i++) {
        keyArr.push({ word: keywords[i], size: keywords[i].length });
    }
    keyArr.sort(compare);
    if (keyArr[0].size <= 4) {
        ...
        for (var i = 0; i < keyArr.length; i++) {
            ...
            if (hasNumber(keyArr[i].word)) { continue; }
...
app.post('/insertItem', (req, res) => {
    ...
    con.query('SELECT ITEM_ID FROM config', (err, rows) => {
        ...
        values = [[[item.ITEM_ID, item.NAME, item.UNIT, item.PACKING, item.VENDOR]]];
...
app.post('/saveOrder', (req, res) => {
    ...
    if (order.UPDATE_MODE == true) {
        ...
        values = [[[order.DATE, order.COMPANY]]];
